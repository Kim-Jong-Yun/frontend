...
function TestPage() {
  ...
  const fetchMonitoredMap = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://3.39.166.207:5557/map/monitored', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const mapData = response.data;
      const mapFileResponse = await axios.get(`http://3.39.166.207:5557/map/file/${mapData._id}`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'blob',
      });
  ...
  const fetchAllTasks = async () => {
    ...
    try {
      const response = await axios.get('http://3.39.166.207:8080/task/task-logs', {
        headers: { Authorization: `Bearer ${token}` },
      });
